...
exports.genInvoice = (req, res) => {
  ...
  ln.invoice({
    ...
    exposeprivatechannels:
      req.body.private === "1" || req.body.private === "true",
  })
    .then((data) => {
...
exports.listInvoice = (req, res) => {
  ...
  if (req.query.label) {
    ...
    ln.listinvoices()
      .then((data) => {
        global.logger.log(
          "Number of Invoices -> " + Object.keys(data.invoices).length
        );
        global.logger.log("listInvoice success");
        res.status(200).json(data);
      })
...
exports.delInvoice = (req, res) => {
  ...
  ln.delinvoice({ label: req.params.label, status: req.params.status })
    .then((data) => {
      res.status(202).json(data);
    })
    .catch((err) => {
...
exports.waitInvoice = (req, res) => {
  ...
  ln.waitinvoice({ label: req.params.label })
    .then((data) => {
