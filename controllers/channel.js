  ...
  ln.listpeers()
    .then((data) => {
      const filteredPeers = data.peers.filter(
        (peer) => peer.channels.length > 0
      );
      Promise.all(
        filteredPeers.map((peer) => {
          // look for a channel that isn't closed already
          const openChan = peer.channels.find(
...
exports.closeChannel = (req, res) => {
  ...
  if (req.query.unilateralTimeout)
    clnReq.unilaterlaltimeout = req.query.unilateralTimeout;
  if (req.query.dest) clnReq.destination = req.query.dest;
  if (req.query.feeNegotiationStep)
    clnReq.fee_negotiation_step = req.query.feeNegotiationStep;
  ...
  ln.listforwards()
    .then(({ forwards }) => {
      ...
      if (reverse === true && forwards.length !== 0) {
        if (offset === 0) offset = forwards.length - offset;
        lastIndex = offset - 1;
