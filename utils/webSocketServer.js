...
const verifyWSClient = (info, next) => {
  try {
    ...
    macaroon
      .importMacaroon(macaroon.base64ToBytes(mac))
      .verify(verRootkey, () => null, []);
    next(true);
  } catch (error) {
...
exports.mountWebServer = (httpServer) => {
  try {
    ...
    wss.on("connection", (websocket, request) => {
      ...
      global.logger.log(
        "Client connected with websocket, total clients: " + wss.clients.size
      );
      ...
      websocket.on("close", () => {
        global.logger.warn(
          "Client disconnected, total clients: " + wss.clients.size
        );
      });
    });
...
exports.listInvoicesToSubscribe = () => {
  ln.listinvoices()
    .then((data) => {
      const max_pay_index = Math.max(
        ...data.invoices.map((inv) => inv.pay_index || 0)
      );
...
exports.subscribeToAnyInvoice = (last_index) => {
  ...
  ln.waitanyinvoice(last_index)
    .then((invUpdate) => {
      global.logger.log(
        "Received Invoice Update: " + JSON.stringify(invUpdate)
      );
